...
import { Lock, CheckCircle2, Copy, ArrowLeft, MessageCircle } from 'lucide-react';
import {
  readUser, createOrder, generateOrderId, EASYPAISA,
  type StoredUser,
} from '@/lib/user';
...
const WHATSAPP_NUMBER = '923397579547'; // +92 339 7579547 — no spaces or +
...
export default function CheckoutPage() {
  ...
  const delivery = freeShip ? 0 : (items.length ? DELIVERY_FEE : 0);
  ...
  // Build WhatsApp message for the delivery slip
    ...
    return (
      <div className="min-h-[70vh] grid place-items-center px-5 py-16">
        <div className="w-full max-w-[480px] bg-white border border-border rounded-2xl p-8 shadow text-center">
          ...
          <Link href="/" className="inline-block mt-3 text-sm text-text-secondary hover:text-brand-accent">
            ← Back to shop
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-[60vh] grid place-items-center px-5 py-16">
        <div className="text-center">
          ...
          <Link href="/" className="inline-block bg-brand-accent text-white px-8 py-3 rounded-full font-semibold">
            Start Shopping
          </Link>
        </div>
      </div>
  ...
  // -------- SUCCESS (no QR — WhatsApp slip only) --------
  if (stage === 'success') {
    return (
      <div className="min-h-[80vh] grid place-items-center px-5 py-16">
        <div className="w-full max-w-[560px] bg-white border border-border rounded-2xl p-8 shadow text-center">
          ...
          <p className="text-xs text-text-secondary mb-6">
            Send your order slip to <strong>+92 339 7579547</strong> so we can process it faster.
          </p>
          ...
          <div className="text-sm text-text-secondary mb-6 text-left bg-orange-50 rounded-xl p-4">
            ...
            <ol className="list-decimal pl-5 space-y-1">
              ...
              <li>
                <strong>Order before 2 PM</strong> for same-day dispatch
              </li>
            </ol>
          </div>
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-8">
            ...
            <form onSubmit={(e) => { e.preventDefault(); setStage('payment'); }} className="bg-white border border-border rounded-2xl p-6 space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+92 3XX XXXXXXX" className={inputCls} required />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Delivery instructions" className={inputCls} />
              </div>
              ...
              <div className="bg-white border-2 border-brand-accent rounded-2xl overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <p className="text-sm text-text-secondary">
                    Send <strong className="text-brand-accent">Rs {total.toLocaleString()}</strong> to the EasyPaisa account below, then enter your Transaction ID (TID) to confirm.
                  </p>
                  <div className="bg-brand-secondary rounded-xl p-4 space-y-3">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="font-bold text-lg">{EASYPAISA.number}</div>
                      </div>
                      <button type="button" onClick={() => copyToClipboard(EASYPAISA.number.replace(/\s/g, ''))} className="text-text-secondary hover:text-brand-accent">
                        <Copy size={18} />
                      </button>
                    </div>
                    <div className="border-t border-border pt-3">
                      <div className="text-xs uppercase tracking-wider text-text-secondary font-semibold mb-1">Account Name</div>
                      <div className="font-semibold">{EASYPAISA.name}</div>
                    </div>
                  ...
                  <div className="bg-yellow-50 rounded-xl p-3 text-xs text-text-secondary">
                    📱 <strong>Step 1:</strong> Open EasyPaisa app → Send Money<br/>
                    📱 <strong>Step 2:</strong> Send Rs {total.toLocaleString()} to the number above<br/>
                    📱 <strong>Step 3:</strong> Copy the TID from your receipt<br/>
                    📱 <strong>Step 4:</strong> Paste TID below and confirm
                  </div>
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider text-text-secondary mb-2">
                      EasyPaisa Transaction ID (TID) *
                    </label>
                    <input value={txnId} onChange={(e) => setTxnId(e.target.value)} placeholder="e.g. 1234567890123" className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-brand-accent outline-none text-sm" required />
                  </div>
                </div>
              </div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit bg-white border border-border rounded-2xl p-6">
          ...
          <div className="space-y-3 max-h-[300px] overflow-y-auto mb-4">
            {items.map((item) => (
              <div key={item.id} className="flex gap-3 text-sm">
                <div className="w-12 h-12 bg-brand-secondary rounded-lg flex items-center justify-center text-lg shrink-0">{item.emoji}</div>
          ...
          <div className="border-t border-border pt-4 space-y-2 text-sm">
            <div className="flex justify-between text-text-secondary"><span>Subtotal</span><span>Rs {subtotal.toLocaleString()}</span></div>
            <div className="flex justify-between text-text-secondary">
              <span>Delivery</span>
              <span className={freeShip ? 'text-brand-success font-semibold' : ''}>{freeShip ? 'FREE' : `Rs ${delivery}`}</span>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-border text-xs text-text-secondary space-y-1">
            ...
            <div>📞 Support: +92 339 7579547</div>
          </div>
        </aside>
      </div>
    </div>
